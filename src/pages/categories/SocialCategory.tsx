...
import { ArrowLeft, Landmark, Trophy, Target, School, ChevronDown, ChevronRight, FolderPlus, ArrowRight } from 'lucide-react';
...
import { socialApCourses, socialSchoolCourses, CourseEntry } from '@/data/category-courses';
...
const SocialCategory = () => {
  ...
  if (selectedSchool && socialSchoolCourses[selectedSchool.id]) {
    schoolCourses.push({ schoolName: selectedSchool.name, courses: socialSchoolCourses[selectedSchool.id] });
  } else {
    Object.entries(socialSchoolCourses).forEach(([schoolId, courses]) => {
        ...
        <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <Link to={`/course-challenge/${course.id}`}>
              <Button variant="outline" className="border-social text-social hover:bg-social hover:text-social-foreground">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {course.units.map((unit) => (
              <Link key={unit.id} to={`/unit/${course.id}/${unit.id}`}>
                <Card className="p-6 hover:shadow-lg transition-all duration-300 hover:scale-105 cursor-pointer border-2 hover:border-social group">
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <NeededCoursesPopup category="social" isOpen={showNeededCourses} onClose={() => setShowNeededCourses(false)} />
      <div className="container mx-auto px-4 py-8 flex-1 max-w-5xl">
        ...
        {socialApCourses.map(renderCourseSection)}
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Create your own custom question banks for any social studies topic.</p>
              </div>
            </div>
