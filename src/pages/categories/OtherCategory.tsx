...
import { ArrowLeft, Sparkles, Trophy, Brain, Target, School, ChevronDown, ChevronRight, FolderPlus, ArrowRight } from 'lucide-react';
...
import { otherApCourses, otherDriversEd, otherSchoolCourses, otherTemporary, CourseEntry } from '@/data/category-courses';

const OtherCategory = () => {
  const navigate = useNavigate();
  const { getWrongAnswerCount, getAllWrongQuestionsForSubject } = useWrongAnswers();
  const shouldShowPopup = usePopupCooldown('other');
  ...
  if (selectedSchool && otherSchoolCourses[selectedSchool.id]) {
    schoolCourses.push({ schoolName: selectedSchool.name, courses: otherSchoolCourses[selectedSchool.id] });
  } else {
    Object.entries(otherSchoolCourses).forEach(([schoolId, courses]) => {
  ...
  const allTopCourses: CourseEntry[] = [...otherApCourses, otherDriversEd];
  if (otherTemporary.units.length > 0) allTopCourses.push(otherTemporary);
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {course.units.map((unit) => (
              <Link key={unit.id} to={`/unit/${course.id}/${unit.id}`}>
                <Card className="p-6 hover:shadow-lg transition-all duration-300 hover:scale-105 cursor-pointer border-2 hover:border-other group">
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <NeededCoursesPopup category="other" isOpen={showNeededCourses} onClose={() => setShowNeededCourses(false)} />
      <div className="container mx-auto px-4 py-8 flex-1 max-w-5xl">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-muted-foreground">AP CSP, Drivers Ed, and miscellaneous</p>
          </div>
        </div>

        {/* AP courses + Drivers Ed */}
        {allTopCourses.map(renderCourseSection)}

        {/* School-Specific Courses (Collapsible) */}
        {schoolCourses.map(({ schoolName, courses }) => courses.length > 0 && (
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Create your own custom question banks for any topic.</p>
              </div>
            </div>
