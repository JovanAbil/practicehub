...
import { ArrowLeft, Calculator, Trophy, Target, School, ChevronDown, ChevronRight, FolderPlus, ArrowRight } from 'lucide-react';
...
import { mathApCourses, mathSchoolCourses, CourseEntry } from '@/data/category-courses';
...
const MathCategory = () => {
  ...
  useEffect(() => {
    if (SCHOOLS.length > 1 && !getSelectedSchool()) {
  ...
  if (selectedSchool && mathSchoolCourses[selectedSchool.id]) {
    schoolCourses.push({ schoolName: selectedSchool.name, courses: mathSchoolCourses[selectedSchool.id] });
  } else {
    Object.entries(mathSchoolCourses).forEach(([schoolId, courses]) => {
      ...
      <div key={course.id} className="mb-12">
        <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
          ...
          <div className="flex gap-2 flex-wrap">
            {wrongCount > 0 && (
              <Button
                onClick={() => navigate(`/quiz/${course.id}/wrong/cram`, { 
                  state: { wrongQuestions: getAllWrongQuestionsForSubject(course.id) } 
                })}
                variant="outline"
            ...
            <Link to={`/course-challenge/${course.id}`}>
              <Button variant="outline" className="border-math text-math hover:bg-math hover:text-math-foreground">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {course.units.map((unit) => (
            <Link key={unit.id} to={`/unit/${course.id}/${unit.id}`}>
              <Card className="p-6 hover:shadow-lg transition-all duration-300 hover:scale-105 cursor-pointer border-2 hover:border-math group h-full">
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <NeededCoursesPopup category="math" isOpen={showNeededCourses} onClose={() => setShowNeededCourses(false)} />
      <div className="container mx-auto px-4 py-8 flex-1 max-w-5xl">
        ...
        <div className="flex items-center gap-4 mb-8 cursor-pointer group" onClick={() => setShowNeededCourses(true)}>
          ...
          <div>
            ...
            <p className="text-muted-foreground">Precalculus, Algebra, and more. Some questions are taken from outside sources such as @FlippedMath and other online resources, this site was just for studying purposes so support the original creators!</p>
          </div>
        </div>

        {/* AP / Universal Courses */}
        {mathApCourses.map(renderCourseSection)}
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Create your own custom question banks for any math topic.</p>
              </div>
            </div>
